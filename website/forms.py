...
def to_lowercase(form, field):
    print('lower')
    if field.data:
        field.data = field.data.lower()

class RegistrationForm(FlaskForm):
    org = StringField('org', validators=[DataRequired(), Length(min=4, max=20), to_lowercase], render_kw={"placeholder":"Organisation"})
    ...
    email = StringField('Email',validators=[DataRequired(), Email(), to_lowercase], render_kw={"placeholder":"Email address"})
